...
void* palloc(pool* frame, void* input, int size) {
	...
	if ((char*)bump < (char*)frame->start) { // bump steps past the start address of our frame
		...
		//return(NULL);
		check = poolRealloc(frame, size);
	} 
	if (!check) { // catches null frames, or failed reallocs
		printf("Pool allocation failed, returning NULL");
		return NULL;
	}
...
int main() {
	...
	int z = 500;
	int* c = (int*)palloc(&intPool, &z, sizeof(int)); // should fail, because pool runs out of space
